...
var Container = function (name, width, height, location) {
  ...
  this.FindSpot = function (item) {
    ...
    for (y = 0; y < this.width - (item.sizex - 1); y += 1) {
      Loop: for (x = 0; x < this.height - (item.sizey - 1); x += 1) {
...
var Storage = new (function () {
  this.Init = function () {
    ...
    this.StashY = me.gametype === 0 ? 4 : 10; // 10 because we are using plugy's 10*10 stash
    ...
    this.Stash = new Container("Stash", 10, this.StashY, 7); // 10 because we are using plugy's 10*10 stash
