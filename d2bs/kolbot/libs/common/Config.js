...
var Config = {
  init: function (notify) {
    ...
    if (FileTools.exists("libs/config/" + configFilename)) {
      ...
      if (notify) {
        print(
          "ÿc1" + classes[me.classid] + "." + me.charname + ".js not found!"
    ...
    try {
      ...
      if (notify) {
        print(
          "ÿc8Error in " +
            e2.fileName.substring(
              e2.fileName.lastIndexOf("\\") + 1,
              e2.fileName.length
            ) +
            "(line " +
            e2.lineNumber +
    ...
    try {
      if (
        ...
        !isIncluded("common/AutoBuild.js") &&
        include("common/AutoBuild.js")
      ) {
        AutoBuild.initialize();
      }
    } catch (e3) {
      print(
        "ÿc8Error in libs/common/AutoBuild.js (AutoBuild system is not active!)"
      );
      print(e3.toSource());
    }
  },
