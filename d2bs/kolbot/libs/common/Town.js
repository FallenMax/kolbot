...
var Town = {
  ...
  doChores: function (repair = false) {
    ...
    var i,
      cancelFlags = [
        0x01, 0x02, 0x04, 0x08, 0x14, 0x16, 0x0c, 0x0f, 0x19, 0x1a,
      ];
  ...
  initNPC: function (task, reason) {
    ...
    switch (task) {
      ...
      case "Key":
        if (
          !getUIFlag(0x0c) &&
          !npc.startTrade(me.act === 3 ? "Repair" : "Shop")
        ) {
          return false;
        }
  ...
  needHealing: function () {
    if (
      (me.hp * 100) / me.hpmax <= Config.HealHP ||
      (me.mp * 100) / me.mpmax <= Config.HealMP
    ) {
      return true;
    }
    ...
    if (
      Config.HealStatus &&
      (me.getState(2) || me.getState(9) || me.getState(61))
    ) {
      return true;
    }
  ...
  buyPotions: function () {
    ...
    for (i = 0; i < 4; i += 1) {
      if (
        ["hp", "mp"].indexOf(Config.BeltColumn[i]) > -1 &&
        col[i] > beltSize - Math.min(Config.MinColumn[i], beltSize)
      ) {
        needPots = true;
      }
    }
    ...
    if (buffer.mp < Config.MPBuffer || buffer.hp < Config.HPBuffer) {
      for (i = 0; i < 4; i += 1) {
        ...
        if (
          col[i] >= beltSize &&
          (!needPots || Config.BeltColumn[i] === "rv")
        ) {
          needBuffer = false;
  ...
  identify: function () {
    ...
    for (i = 0; i < list.length; i += 1) {
      ...
      if (
        (!list[i].getFlag(0x10) || Config.LowGold > 0) &&
        ([-1, 4].indexOf(Pickit.checkItem(list[i]).result) > -1 ||
          (!list[i].getFlag(0x10) && Item.hasTier(list[i])))
      ) {
        break;
      }
    }
    ...
    MainLoop: while (list.length > 0) {
      ...
      if (
        !item.getFlag(0x10) &&
        item.location === 3 &&
        this.ignoredItemTypes.indexOf(item.itemType) === -1
      ) {
  ...
  fieldID: function () {
    // not exactly a town function but whateva
      ...
      if (result.result === -1) {
        ...
        switch (result.result) {
          case 0:
            ...
            if (
              Config.DroppedItemsAnnounce.Enable &&
              Config.DroppedItemsAnnounce.Quality.indexOf(item.quality) > -1
            ) {
              say(
                "Dropped: [" +
                  Pickit.itemQualityToName(item.quality)
                    .charAt(0)
                    .toUpperCase() +
                  Pickit.itemQualityToName(item.quality).slice(1) +
                  "] " +
                  item.fname
                    .split("\n")
                    .reverse()
              ...
              if (
                Config.DroppedItemsAnnounce.LogToOOG &&
                Config.DroppedItemsAnnounce.OOGQuality.indexOf(item.quality) >
                  -1
              ) {
  ...
  shopItems: function () {
    ...
    for (i = 0; i < items.length; i += 1) {
      ...
      if (result.result === 1 && Item.autoEquipCheck(items[i])) {
        try {
          if (
            Storage.Inventory.CanFit(items[i]) &&
    ...
    if (this.gambleIds.length === 0) {
      ...
      for (i = 0; i < Config.GambleItems.length; i += 1) {
        if (isNaN(Config.GambleItems[i])) {
          if (
            NTIPAliasClassID.hasOwnProperty(
              Config.GambleItems[i].replace(/\s+/g, "").toLowerCase()
            )
          ) {
            this.gambleIds.push(
              NTIPAliasClassID[
            ...
            Misc.errorReport(
              "ÿc1Invalid gamble entry:ÿc0 " + Config.GambleItems[i]
            );
          }
        } else {
  ...
  cubeRepair: function () {
    ...
    items.sort(function (a, b) {
      return (
        (a.getStat(72) * 100) / a.getStat(73) -
        (b.getStat(72) * 100) / b.getStat(73)
      );
    });
            ...
            if (cubeItems[0].bodylocation === bodyLoc) {
              print(
                cubeItems[0].fname
                  ...
                  .join(" ")
                  .replace(/ÿc[0-9!"+<;.*]/, "")
                  .trim() + " successfully repaired and equipped."
              );
            ...
            switch (item.itemType) {
              ...
              case 87: // Amazon javelins
                ...
                if (
                  typeof quantity === "number" &&
                  (quantity * 100) /
                    (getBaseStat("items", item.classid, "maxstack") +
                      item.getStat(254)) <=
                    repairPercent
                ) {
              ...
              default:
                ...
                if (
                  ...
                  (durability * 100) / item.getStat(73) <= repairPercent
                ) {
                  itemList.push(copyUnit(item));
                }
            ...
            if (typeof charge === "object") {
              if (charge instanceof Array) {
                for (i = 0; i < charge.length; i += 1) {
                  if (
                    charge[i] !== undefined &&
                    charge[i].hasOwnProperty("charges") &&
                    (charge[i].charges * 100) / charge[i].maxcharges <=
                      repairPercent
                  ) {
                    itemList.push(copyUnit(item));
                  }
                ...
                (charge.charges * 100) / charge.maxcharges <=
                repairPercent
              ) {
                itemList.push(copyUnit(item));
              }
  ...
  reviveMerc: function () {
    ...
    if (!!me.getMerc()) {
      if (Config.MercWatch) {
        // Cast BO on merc so he doesn't just die again
  ...
  needMerc: function () {
    ...
    if (me.gametype === 0 || !Config.UseMerc || me.gold < me.mercrevivecost) {
      // gametype 0 = classic
      return false;
    }
  ...
  canStash: function (item) {
    ...
    if (
      this.ignoredItemTypes.indexOf(item.itemType) > -1 ||
      ignoredClassids.indexOf(item.classid) > -1 ||
      !Storage.Stash.CanFit(item)
    ) {
      return false;
    }
  ...
  stash: function (stashGold) {
    ...
    if (items) {
      ...
      if (inserted) {
        say("/sp 1"); // swap to make new empty page the first page
      }
    }
    ...
    if (stashGold) {
      if (
        me.getStat(14) >= Config.StashGold &&
        me.getStat(15) < 25e5 &&
        this.openStash()
      ) {
  ...
  clearInventory: function () {
    ...
    for (i = 0; !!items && i < items.length; i += 1) {
      if (
        [18, 41, 76, 77, 78].indexOf(items[i].itemType) === -1 && // Don't drop tomes, keys or potions
        ...
        switch (result) {
          case 0: // Drop item
            if (
              (getUIFlag(0x0c) || getUIFlag(0x08)) &&
              (items[i].getItemCost(1) <= 1 || items[i].itemType === 39)
            ) {
  ...
  initialize: function () {
    ...
    switch (me.act) {
      case 1:
        ...
        fire = [
          fireUnit.roomx * 5 + fireUnit.x,
          fireUnit.roomy * 5 + fireUnit.y,
        ];
  ...
  move: function (spot) {
    ...
    if (
      ...
      path = [
        5113, 5068, 5108, 5051, 5106, 5046, 5104, 5041, 5102, 5027, 5098, 5018,
      ];
  ...
  moveToSpot: function (spot) {
    ...
    if (
      !this.act[me.act - 1].hasOwnProperty("spot") ||
